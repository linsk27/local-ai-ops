...
## v0.3.0 Local Intelligence

- 本地知识库：基于本地数据库中的资产、监控、告警、续费、SSH/宝塔配置状态进行问答，不默认调用外部 AI。
- 资产关系图：展示显式保存的资产关系，并根据 DNS 记录、域名和服务器 IP 推断常见依赖关系。
- 续费中心：汇总到期时间、自动续费状态、数据来源和阿里云控制台入口，帮助集中处理续费风险。
- 数据边界：这些页面读取本地数据库；只有用户点击“同步资产”时才会调用阿里云 OpenAPI。
